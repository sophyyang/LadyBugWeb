			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="categoryId" title="${message(code: 'status.categoryId.label', default: 'Category Id')}" />
					
						<g:sortableColumn property="description" title="${message(code: 'status.description.label', default: 'Description')}" />
					
						<g:sortableColumn property="iOrder" title="${message(code: 'status.iOrder.label', default: 'IO rder')}" />
				...
				<g:each in="${statusInstanceList}" status="i" var="statusInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${statusInstance.id}">${fieldValue(bean: statusInstance, field: "categoryId")}</g:link></td>
					
						<td>${fieldValue(bean: statusInstance, field: "description")}</td>
					
						<td>${fieldValue(bean: statusInstance, field: "iOrder")}</td>
